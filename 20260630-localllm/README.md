...
- Alibaba Qwenの方が、コーディング力は高いらしい
- Gemma 4の方ガ、アシスタントとしての汎用力に優れるらしい
- `Gemma 4 26B-A4B` がモデルも大きく、計算速度もあるの良さそう
    - お家AIアシスタントは `Gemma 4 26B-A4B` を採用中
...
いくつかパラメータを指定して、応答速度と、賢さを調整する必要がある。

#### コンテキストサイズ

llama-server起動時に、最大コンテキストサイズを指定するパラメータがある。
llama-serverの場合、指定しないと最大サイズが使われる。

#### 出力トークンサイズ

リクエスト時には出力トークンサイズを指定して、出力サイズを調整する必要がある。
OpenAI互換APIの場合、`max_tokens` のパラメータで渡す。

#### Reasoningサイズ

Reasoningの具合は、llama-server起動時の `--reasoning-budget` で指定する。
`max_tokens`のうち、この範囲のトークンをreasoning-bedgetで使われるようである。

リクエスト時に `"reasoning_effort": "none"` を指定すればユーザ向け出力から入って速く見える。
...
LangChainのcreate_agentを使うと複数の推論バックエンドでエージェントが作れるが、コンテキスト圧縮がミドルウェアとして提供されており、使うことができる。
実行するトリガーとなるトークン、メッセージ数、プロンプトをパラメータにする。
